	...
	return (
		<div>
			<Label>
				Copper Type{" "}
				<HelpPopover>
					Electro-Deposited (ED) copper is brittle. ED copper can be deposited in thinner layers , which can
					be beneficial in applications where thinner traces are required, and electrodeposited copper is less
					expensive.
				</HelpPopover>
			</Label>
			<Listbox
				...
				<div className="relative">
					<Listbox.Button className="flex h-9 w-full items-center justify-between rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50">
						<span className="block truncate">{copperType}</span>
