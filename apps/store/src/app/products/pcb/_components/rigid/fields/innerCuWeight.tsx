...
import { setInnerCuWeight, setPcbPrice } from "@/redux/reducers/rigidPcbSlice";
...
export default function InnerCuWeight() {
  ...
  const innerCuWeightOptions = useSelector(
    (state: ReduxState) => state.rigidPcb.innerCuWeightOptions,
  );
  const innerCuWeight = useSelector(
    (state: ReduxState) => state.rigidPcb.innerCuWeight,
  );
      ...
      >
        <div className="relative">
          <Listbox.Button className="border-input ring-offset-background placeholder:text-muted-foreground focus:ring-ring flex h-9 w-full items-center justify-between rounded-md border bg-transparent px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 disabled:cursor-not-allowed disabled:opacity-50">
            <span className="block truncate">{innerCuWeight}</span>
