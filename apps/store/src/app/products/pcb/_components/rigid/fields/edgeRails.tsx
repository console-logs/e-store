...
export default function EdgeRails() {
  ...
  const edgeRailOptions = useSelector(
    (state: ReduxState) => state.rigidPcb.edgeRailsOptions,
  );
  const edgeRails = useSelector(
    (state: ReduxState) => state.rigidPcb.edgeRails,
  );
    ...
    <div
      hidden={
        designFormat === "Single PCB"
          ? true
          : designFormat === "Panel by Customer"
            ? true
            : false
      }
    >
