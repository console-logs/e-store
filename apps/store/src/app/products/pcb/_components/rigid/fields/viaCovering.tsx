...
import { setPcbPrice, setViaCovering } from "@/redux/reducers/rigidPcbSlice";
...
export default function ViaCovering() {
  ...
  const viaCoveringOptions = useSelector(
    (state: ReduxState) => state.rigidPcb.viaCoveringOptions,
  );
  const viaCovering = useSelector(
    (state: ReduxState) => state.rigidPcb.viaCovering,
  );
  ...
  return (
    <div hidden={hiddenOptions.includes(baseMaterial) ? true : false}>
      <Label>
